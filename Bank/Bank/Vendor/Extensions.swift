...
func apply (_ string: NSMutableAttributedString, word: String) -> NSMutableAttributedString {
    let range = (string.string as NSString).range(of: word)
    return apply(string, word: word, range: range, last: range)
}

func apply (_ string: NSMutableAttributedString, word: String, range: NSRange, last: NSRange) -> NSMutableAttributedString {
    if range.location != NSNotFound {
        string.addAttribute(NSForegroundColorAttributeName, value: UIColor(hex: 0xe5700a), range: range)
        ...
        _ = apply(string, word: word, range: newRange, last: range)
    }
    return string
}
...
extension String {
    ...
    func aesEncrypt(_ key: String, ivSalt: String) -> String {
        assert(key.characters.count == 16, "invalid key value")
        return (self as NSString).aes128Encrypt(withKey: key, andIv: ivSalt)
    }
    
    func aesDecrypt(_ key: String, ivSalt: String) -> String {
        return (self as NSString).aes128Decrypt(withKey: key, andIv: ivSalt)
    }
